...
def three_chapters(path, rep, sbareme, _class, semester, level, academie, direction, school, subject, teacher):
    ...
    # first chapter horizontal bar chart
    plt.figure(2)
    ax = df.plot.barh(x='Nom Complet', y=df_top[0], rot=1, align='edge', width=0.5, figsize=(10, 15),
                      color='#0DA89F', edgecolor="#000000")
    ax.grid(axis='x')
    plt.xticks(rotation=45, fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylabel(None)
    ch1barpath = './report/3c/2.png'
    plt.savefig(ch1barpath, dpi=120, bbox_inches='tight')
    plt.close()

    df = df.sort_values(df_top[1], ascending=True)
    df = df.reset_index(drop=True)

    # second chapter horizontal bar chart
    plt.figure(3)
    ax = df.plot.barh(x='Nom Complet', y=df_top[1], rot=1, align='edge', width=0.5, figsize=(10, 15),
                      color='#0DA89F', edgecolor="#000000")
    ax.grid(axis='x')
    plt.xticks(rotation=45, fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylabel(None)
    ch2barpath = './report/3c/3.png'
    plt.savefig(ch2barpath, dpi=120, bbox_inches='tight')
    plt.close()

    df = df.sort_values(df_top[2], ascending=True)
    df = df.reset_index(drop=True)

    # third chapter horizontal bar chart
    plt.figure(4)
    ax = df.plot.barh(x='Nom Complet', y=df_top[2], rot=1, width=0.5, figsize=(10, 15),
                      color='#0DA89F', edgecolor="#000000")
    ax.grid(axis='x')
    plt.xticks(rotation=45, fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylabel(None)
    ch3barpath = './report/3c/4.png'
    plt.savefig(ch3barpath, dpi=120, bbox_inches='tight')
    plt.close()
    del ax
    del df
    ...
    d1.text((1067, 131), directionh, font=regular, fill=(0, 0, 0))
    ...
    paths = ['./sheetz/1.png', './sheetz/2.png', './sheetz/3.png', './sheetz/4.png', './sheetz/5.png', './sheetz/6.png']
    i1.save(paths[0])
    del i1
    del d1
    ...
    # 4th page chapter one
    i4 = Image.open('./model/report/4.png')
    ch1 = Image.open(ch1barpath)
    i4.paste(ch1, (188, 296))
    i4.save(paths[3])
    del i4
    del ch1

    # 5th page chapter two
    i5 = Image.open('./model/report/5.png')
    ch2 = Image.open(ch2barpath)
    i5.paste(ch2, (188, 310))
    i5.save(paths[4])
    del i5
    del ch2

    # 6th page chapter three
    i6 = Image.open('./model/report/6.png')
    ch3 = Image.open(ch3barpath)
    i6.paste(ch3, (188, 310))
    i6.save(paths[5])
    del i6
    del ch3
    i = 1
...
def two_chapters(path, rep, sbareme, _class, semester, level, academie, direction, school, subject, teacher):
    df = pd.read_csv(path, delimiter=',')
    del df['Code Massar']
    del df["Numéro d'ordre"]
    del df['Classe']
    ...
    paths = ['./sheetz/1.png', './sheetz/2.png', './sheetz/3.png', './sheetz/4.png', './sheetz/5.png']
    i1.save(paths[0])
    del i1
    del d1
...
def one_chapter(path, rep, sbareme, _class, semester, level, academie, direction, school, subject, teacher):
    df = pd.read_csv(path, delimiter=',')
    del df['Code Massar']
    del df["Numéro d'ordre"]
    del df['Classe']
    ...
    paths = ['./sheetz/1.png', './sheetz/2.png', './sheetz/3.png', './sheetz/4.png']
    i1.save(paths[0])
    del i1
    del d1
